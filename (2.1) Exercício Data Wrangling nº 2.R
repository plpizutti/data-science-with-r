...
# table traz uma tabela de distribuição de frequências por categoria da variável escolhida
...
base_covid <- base_covid[!(base_covid$nome=="Other"),] # excluída por seu nome (desta forma está mantendo no dataset todas as linhas que não contém "Other" no nome)
...
base_covid <- base_covid %>% mutate(tipo_transmissao = recode(tipo_transmissao,
                            ...
                            mutate(grupos = cut(casos_relativo, #cria a variavel chamada grupos com base na casos_relativo
